...
public class HttpUrlSource implements Source {
    ...
    public void close() throws ProxyCacheException {
        if (connection != null) {
            try {
                ...
                CacheLog.error("Error closing connection correctly. Should happen only on Android L. " +
    ...
    private void fetchContentInfo() throws ProxyCacheException {
        CacheLog.debug("Read content info from " + sourceInfo.url);
        ...
        try {
            ...
            this.sourceInfo = new SourceInfo(sourceInfo.url, length, mime);
            this.sourceInfoStorage.put(sourceInfo.url, sourceInfo);
            CacheLog.debug("Source info fetched: " + sourceInfo);
        } catch (IOException e) {
            CacheLog.error("Error fetching info from " + sourceInfo.url, e);
        } finally {
    ...
    private HttpURLConnection openConnection(long offset, int timeout) throws IOException, ProxyCacheException {
        ...
        do {
            CacheLog.debug("Open connection " + (offset > 0 ? " with offset " + offset : "") + " to " + url);
