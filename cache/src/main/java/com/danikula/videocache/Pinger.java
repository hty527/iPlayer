...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                CacheLog.warn("Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                CacheLog.error("Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        CacheLog.error(error, new ProxyCacheException(error));
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            CacheLog.info("Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            CacheLog.error("Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
